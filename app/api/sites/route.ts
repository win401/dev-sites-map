...
export async function GET(req: Request) {
  ...
  if (minLng === null || minLat === null || maxLng === null || maxLat === null) {
    return NextResponse.json(
      {
        ok: false,
        error: "Missing/invalid params. Use ?minLng=...&minLat=...&maxLng=...&maxLat=...",
        version: "db-bbox-v1",
      },
      { status: 400 }
    );
  ...
  if (minLng >= maxLng || minLat >= maxLat) {
    return NextResponse.json(
      { ok: false, error: "Invalid bbox range", version: "db-bbox-v1" },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: true, count: rows.length, sites: rows, version: "db-bbox-v1" });
  } catch (e: any) {
    return NextResponse.json(
      { ok: false, error: e?.message ?? String(e), version: "db-bbox-v1" },
      { status: 500 }
    );
  } finally {
